...
export const createDiceRollMessage = async (
  ...
  // Crie uma mensagem descritiva para o conteúdo
  ...
  if (sessionId) {
    await supabase.from('messages').insert({
      content: rollDescription, // Use a descrição em vez de string vazia
...
export const createAbilityMessage = (
  ...
  if (!isDeactivation) {
    ...
    description.push(
      '',
      `**▸ Custo:** ${costAbility} ${cost.type === 'mp' ? '✨ Mana' : '💪 Fadiga'}`,
      ''
    );
    
    const manaAfterCost = character.currentMp - cost.mpCost;
    const fatigueAfterCost = character.fatigue + cost.fatigueCost;

    if (cost.type === 'mp') {
      description.push(`**▸ Mana Restante:** ${manaAfterCost} ✨`);
    } else if (cost.type === 'fatigue') {
      description.push(`**▸ Fadiga Atual:** ${fatigueAfterCost} 💪`);
    } else if (cost.type === 'both') {
      description.push(`**▸ Fadiga Atual:** ${fatigueAfterCost} 💪`);
      description.push(`**▸ Mana Restante:** ${manaAfterCost} ✨`);
    }
