...
import { calculateBonus, calculatePhysicalDamage, calculateMagicDamage, calculateSpeed } from '../utils/character';
...
export const ChatSystem: React.FC<ChatSystemProps> = ({ character }) => {
  ...
  // Update presence when character stats change
  useEffect(() => {
    ...
    const physicalDamage = calculatePhysicalDamage(character.attributes.str + bonus.str);
    const magicDamage = calculateMagicDamage(character.attributes.int + bonus.int).toString();
    const speed = calculateSpeed(character.attributes.agi + bonus.agi);
