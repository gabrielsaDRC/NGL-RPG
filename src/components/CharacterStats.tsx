...
import { Heart, Sparkles, Sword, Flame, Skull, Shield, Wind } from 'lucide-react';
...
export const CharacterStats: React.FC<CharacterStatsProps> = ({
  ...
  const handleFatigueChange = (newValue: number) => {
    const clampedValue = Math.max(0, Math.min(100, newValue));
    onFatigueChange(clampedValue);
  };
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-[#00ffe1]/70 text-center mt-1">Ataque Físico</div>
          </div>
        </div>
      ...
      {/* Fatigue Row */}
      <div className="flex items-center space-x-4">
        <Skull size={20} className="text-[#00ffe1] drop-shadow-[0_0_5px_#00ffe1]" />
        <div className="flex-1">
          <div className="flex items-center justify-between bg-[#1a1a1a] border border-[#00ffe1] rounded-lg p-2">
            <button
              onClick={() => handleFatigueChange(stats.fatigue - 5)}
              className="text-[#00ffe1] hover:text-[#00ff88] px-1"
              disabled={readOnly}
            >
              -
            </button>
            <span className={getFatigueColor(stats.fatigue)}>
              {stats.fatigue}
            </span>
            <button
              onClick={() => handleFatigueChange(stats.fatigue + 5)}
          ...
          <div className="w-full bg-[#1a1a1a] rounded-full h-1 mt-1 border border-[#00ffe1] overflow-hidden">
            <div
              className={`h-full ${getFatigueColor(stats.fatigue)} transition-all duration-300`}
              style={{ width: `${stats.fatigue}%`, backgroundColor: 'currentColor' }}
            />
          </div>
        </div>
      </div>
